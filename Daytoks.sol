...
contract DayToks is ERC721, ERC721Burnable, ERC721URIStorage, Ownable {
    
    constructor() ERC721("ToDaysToks", "DAYTOKS") {}

    function safeMint(address to, uint256 tokenId, bytes32 swarmHash) public onlyOwner {
   ...
   function bytes32ToString(bytes32 _bytes32) private pure returns (string memory) {
    ...
    // Función que permite quemar una unidad de token
    function burn(uint256 id) public override {
        // Verifica que el usuario que llama a la función sea el dueño actual de la unidad de token
        require(_ownerOf(id) == msg.sender, "No eres el poseedor de esta unidad de token");
        // Quema la unidad de token con el ID especificado
        _burn(id);
    }
